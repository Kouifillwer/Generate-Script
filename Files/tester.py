...
for get_name in myDict:
    ...
    for i in range(0, number_of_list):
        ...
        new_neighbor = m_neighbor.replace("[NBC_ARFCN_LIST]", f"{str(myDict[f'{get_name}']['NeighborCellARFCN'])[1:-1]}")
        new_neighbor = new_neighbor.replace(f"[HONeighborCellID]", f"{HONeighborCellID}")
        new_neighbor = new_neighbor.replace("[NeighborCellID]", f"{str(myDict[f'{get_name}']['NeighborCellID'][i])}")
        new_neighbor = new_neighbor.replace("[NeighborCellBSIC]", f"{str(myDict[f'{get_name}']['NeighborCellBSIC'][i])}")
        new_neighbor = new_neighbor.replace("[NeighborCellARFCN]", f"{str(myDict[f'{get_name}']['NeighborCellARFCN'][i])}")
        new_neighbor = new_neighbor.replace("[NeighborCellLAC]", f"{str(myDict[f'{get_name}']['NeighborCellLAC'][i])}")
        print(i)
        print(new_neighbor)
        pass
